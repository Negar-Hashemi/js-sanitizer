...
(function ensureVitest() {
  // Ensure Vitest itself if requested and missing
  if (!hasDep('vitest')) {
  ...
  // Ensure vite-plugin-babel (but continue even if it’s missing)
...
`;
  ...
  plugins: [ babel({ filter: /(\\.test|\\.spec)\\.[jt]sx?$/, babelConfig: { configFile: true } }) ]`);
      ...
      log(`Found ${path.basename(existing)}. Skipping plugin wiring since vite-plugin-babel is not installed.`);
    }
  } else {
    ...
    log(`Created ${path.basename(out)}${hasBabelPlugin ? ' enabling Babel via vite-plugin-babel' : ''}`);
  }

  // Ensure a setup file entry exists (harmless)
