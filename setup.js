...
// Robust ignore logic (Windows-safe); never transform the sanitizer package itself
...
// Keep Rollup fixtures untouched so “helpful error” tests behave
...
// Deterministic options; IMPORTANT: do NOT set 'plugins' here.
// We rely on the project Babel config (which setup.js ensures contains 'module:js-sanitizer').
const BASE_OPTS = {
  babelrc: true,
  configFile: true,
  rootMode: 'upward-optional',
  ...
  // Extra guard: even if project config tries, do not transform our own plugin
...
(async () => {
  ...
  try {
    ...
    if (typeof register === 'function') {
      ...
      const parentURL = pathToFileURL(process.cwd() + path.sep).href; // Windows-safe file://
