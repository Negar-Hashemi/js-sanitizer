...
/* ----------------------------------------
 * 4) Vitest wiring (ensure setup file always; config/plugin only when relevant)
 * --------------------------------------*/
(function ensureVitest() {
  // Detect whether this workspace actually runs Vitest even if not listed in deps
  ...
  // 0) ALWAYS ensure package.json -> vitest.setupFiles and the setup file itself.
  updatePkg((p) => {
    p.vitest = p.vitest || {};
...
`);
  ...
  // If this workspace doesn't actually run Vitest, stop here.
  if (!vitestReferenced) {
    log('Vitest not detected in this workspace (no dep and test script does not reference vitest) — created vitest.setup.js and setupFiles only; skipped plugin/config.');
    return;
  }

  // From here on, the workspace runs Vitest: optionally ensure vite-plugin-babel
  ...
  // If the workspace already has a Vitest config (ts/js/mjs), DO NOT edit it.
  ...
  if (existing) {
    log(`${path.basename(existing)} present in this workspace; not modifying. If you need Babel transforms, add vite-plugin-babel there.`);
    return;
  }

  // No config present here — generate a minimal self-contained config
  // NOTE: do NOT touch configs in other packages; this is local to the current workspace.
  const isESMProject = (readJSON(PKG_PATH)?.type === 'module');
  const outPath = isESMProject ? path.join(ROOT, 'vitest.config.js') : path.join(ROOT, 'vitest.config.mjs');
...
export default defineConfig(async () => {
  ...
  try {
    ...
    // vite-plugin-babel not installed — tests still run; install it to enable Babel transforms
  }
