...
/* ----------------------------------------
 * 4) Vitest wiring
 *    Prefer vite-plugin-babel; fallback to @vitejs/plugin-react if available
 * --------------------------------------*/
(function ensureVitest() {
  ...
  if (!hasViteBabel && !hasViteReact) {
    log('WARNING: To enable js-sanitizer in Vitest, install ONE of:\n  npm i -D vite-plugin-babel\n  or\n  npm i -D @vitejs/plugin-react');
    return;
  }

  // Choose config filename: ESM projects use .js, CJS projects use .mjs
  ...
  if (exists(outPath)) {
    ...
    if (hasViteBabel && /vite-plugin-babel/.test(src)) {
      log(`${path.basename(outPath)} already references vite-plugin-babel`);
      return;
    }
    if (!hasViteBabel && hasViteReact && /@vitejs\/plugin-react/.test(src) && /babel:/.test(src)) {
      log(`${path.basename(outPath)} already configures @vitejs/plugin-react with a babel section`);
      return;
    }
    // If an incompatible/complex config exists, do not overwrite—log helpful snippet
    log(`Found ${path.basename(outPath)} but did not auto-edit. Please add one of the following manually:`);
  ...
  export default defineConfig({
    plugins: [
      react({
        babel: {
          // Use your babel.config file with js-sanitizer
          // configFile: true, // (if you want to reuse the file)
  ...
  // Generate a fresh vitest config using whichever plugin is available
  let content;
  if (hasViteBabel) {
    content = `// Auto-generated by js-sanitizer setup
import { defineConfig } from 'vitest/config'
import babel from 'vite-plugin-babel'

export default defineConfig({
  test: {
    // put your test options here if needed
  },
  plugins: [
    // Run before other transforms so docblocks remain
    { enforce: 'pre', ...babel({
      filter: /(\\.test|\\.spec)\\.[jt]sx?$/,
      // Reuse file-based Babel config created by setup (modules:false + js-sanitizer)
      babelConfig: { configFile: true }
    }) }
  ]
})
`;
    ...
    // Fallback for React users: piggyback on the official plugin's Babel hook
    content = `// Auto-generated by js-sanitizer setup
import { defineConfig } from 'vitest/config'
import react from '@vitejs/plugin-react'
...
`;
  ...
  log(`Created ${path.basename(outPath)} to enable Babel for Vitest (${hasViteBabel ? 'vite-plugin-babel' : '@vitejs/plugin-react'})`);

  // Optional: setup file (harmless)
...
`);
  ...
  pkg.vitest = vit;
  writeJSON(PKG_PATH, vit);
  // NOTE: we updated the 'vitest' key above; now write full package.json back
  const fullPkg = readJSON(PKG_PATH) || {};
  fullPkg.vitest = vit;
