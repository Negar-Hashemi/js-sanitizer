...
(function ensureMocha() {
  ...
  if (!hasAnyDep('@babel/register')) tryInstall(['@babel/register'], true);
...
`;
  ...
  //    - Enables @babel/register for CJS-loaded tests (keeps project babel config)
...
try {
  require('@babel/register')({
    ...
    rootMode: 'upward-optional',
    cache: true
    // Do NOT force transform-modules-commonjs here; we respect project ESM.
    // The sanitizer plugin is injected via project babel config (setup ensured it).
  });
  console.log('[js-sanitizer]: @babel/register active (global)');
} catch (e) {
  console.warn('[js-sanitizer]: Could not enable @babel/register:', e && e.message || e);
}

try {
  const { register } = require('node:module');
  const loaderUrl = pathToFileURL(path.join(__dirname, 'sanitizer.esm.loader.mjs')).href;
